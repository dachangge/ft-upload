...
class UploadHelper{
  ...
  async submit(file: UploadFile):Promise<String | void | UploadFile> {
    ...
    let bucketType = this.params.bucketType;
    if (!bucketType) {
      bucketType = +(file.size > middleSize)
    }
    // 直传
    if (file.size < middleSize) {
        return axios.get(this.config.directUrl, { params: { ...this.params, bucketType } }).then(res => {
          // 阿里直传
    ...
    // 分片上传
      ...
      return axios.get(this.config.secretKeyUrl, { params: { ...this.params, bucketType } }).then(res => {
        // alicloud 分片
        if (this.config.version  === UploadVersion.alicloud) {
              const ossConfig:any = {
              ...
              return axios.get(this.config.directUrl, { params: { ...this.params, bucketType } }).then(res1 => {
